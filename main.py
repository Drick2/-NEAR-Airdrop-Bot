...
def get_task_keyboard():
    buttons = [
        ...
        [InlineKeyboardButton("✅ I've Joined Both", callback_data='verify_tasks')]
    ]
...
async def add_points_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for uid in user_ids:
        if get_user(uid):
            ...
            try:
                await context.bot.send_message(uid, f"🎁 You received {points} $NEAR from admin!")
            except:
                pass
            success += 1
        else:
...
async def add_points_all_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for (uid,) in users:
        add_points(uid, points)
        try:
            await context.bot.send_message(uid, f"🎁 All users received {points} $NEAR from admin!")
        except:
            pass
        count += 1
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if referrer_id and referrer_id != user_id and get_user(referrer_id):
            ...
            try:
                await context.bot.send_message(
                    referrer_id,
                    f"🎉 You gained a new referral! +200 $NEAR. Total: {get_user(referrer_id)[4]}"
                )
            except:
                pass
        else:
    ...
    welcome_text = (
        ...
        f"━━━━━━━━━━━━━━━━━━━━━\n"
        f"**💰 REWARD STRUCTURE**\n"
        f"• **1,000 $NEAR** (~$4) – Complete verification\n"
        f"• **200 $NEAR** per referral – Unlimited\n"
        f"• **150 $NEAR** daily – Claim every 24h\n"
        f"• **Extra 50 $NEAR** – Shout 'NEAR TO THE MOON' in group\n"
        f"━━━━━━━━━━━━━━━━━━━━━\n\n"
        f"**📋 MANDATORY STEPS**\n"
        f"✅ Join our Telegram Group\n"
        f"✅ Join our Telegram Channel\n"
        f"✅ Submit your BSC wallet address\n\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == 'join_airdrop':
        if tasks_completed:
            await query.edit_message_text("✅ You already completed verification! Now refer friends.")
            return
        await query.edit_message_text(
            "**📋 VERIFICATION STEPS**\n\n"
            "1️⃣ Join Group\n2️⃣ Join Channel\n3️⃣ Submit your Telegram username\n4️⃣ Submit your BSC wallet\n\n"
            "Click below to join:",
        ...
        await query.edit_message_text("❌ Wallet submission cancelled. Use /start to restart.")
        context.user_data['state'] = None
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.photo:
        if pending_ss:
            ...
            caption = f"📸 Screenshot from user {user_id} (@{update.effective_user.username})\nName: {db_first_name}\nUse /addpoints {user_id} <points> to reward."
    ...
    if text == "💰 Balance":
        ...
        await update.message.reply_text(
            ...
            f"Share this link – you get **200 $NEAR** for each friend who joins!\n"
            f"Total referrals so far: `{referrals}`",
            parse_mode='Markdown'
        )
    elif text == "📤 Withdraw":
        await send_event_image('withdraw', user_id, context, "📤 **Withdrawals**")
        await update.message.reply_text(
            "**📤 Withdrawals**\n\n"
        ...
        if last_daily == today:
            await update.message.reply_text("⏳ You've already claimed today's bonus. Come back tomorrow!")
        else:
            bonus = 150
